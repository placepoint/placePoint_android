...
class RegisterPresenter(var view: RegisterHelper) {
    fun getRegisterData(userType: String) {
...
//        }
   if (type == "3"||type=="4") {
    ...
    private fun registerWebService(email: String, pass: String, bName: String, bLoc: String, bCat: String, auth_code: String, type: String) {
        view.showLoader()
        val retrofit = Constants.getWebClient()
        val service = retrofit!!.create(Service::class.java)
        val call: Call<ResponseBody> = service.postRegisterData(email, pass, bName, bLoc, bCat, auth_code, type, "")
